...
// angle为角度制, way为路数
void createHoming(entt::registry& registry, int way, float angle)
{
    ...
    if (way == 1)
    {
        BulletSystem::createBullet(registry, {x, y}, speed * toPlayer);
        return;
    }

    // 计算基础方向角度
    const float toPlayerAngle = atan2(toPlayer.y, toPlayer.x);
    const float startAngle = toPlayerAngle - angle * 0.5f;

    // 计算角度步长（对称分布）
    const float angleStep = angle / (way - 1);
    for (int i = 0; i < way; i++)
    {
        const float currentAngle = startAngle + angleStep * i;
        glm::vec2 dir(cos(currentAngle), sin(currentAngle));
        BulletSystem::createBullet(registry, {x, y}, speed * dir);
    }
}
}
