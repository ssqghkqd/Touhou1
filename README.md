# 东方同人游戏——Touhou1 / Touhou Fan Game - Touhou1

## 制作人员名单 / Credits
`程序 - ss` / `Programming - ss`  
`美术 - 盖饭` / `Art - 盖饭`

## 简介 / Introduction
由两个高中生制作的游戏 / A game created by two high school students  
游戏名称 Touhou 1 ~ religious war 东方宗教之战 / Game Title: Touhou 1 ~ Religious War  
类型 东方同人STG游戏 / Genre: Touhou Fan-made STG Game

## 开发环境介绍 / Development Environment
C++23标准 OpenGL 3.3 / C++23 Standard OpenGL 3.3

## 构建方法 / Building Instructions
cmake>=3.28 clang>=18/gcc>=14  
具体命令行构建怎么我也不知道 因为我平时用ide点 反正查一下cmake命令help吧 /
I don't know the exact command line build process as I usually use IDE, just check cmake command help

## 使用的库 / Libraries Used
`glfw` `stb_image` `glm` `entt` `miniaudio` `spdlog` `nlohmann_json`

## 许可证说明 / License Information
- **代码部分**：GPLv3（详见 [LICENSE.md](LICENSE.md)） / **Code**: GPLv3 (see [LICENSE.md](LICENSE.md))
- **美术资源**：版权归美术组所有，**不随源码分发** / **Art Assets**: Copyright reserved by art team, **not included in source distribution**
- **第三方库**：详见 [THIRD-PARTY-LICENSES.md](THIRD-PARTY-LICENSES.md) / **Third-party Libraries**: See [THIRD-PARTY-LICENSES.md](THIRD-PARTY-LICENSES.md)

**链接** / **Link**: [两个高中生的东方同人游戏](https://www.bilibili.com/video/BV1391vBAEuz) / [Touhou Fan Game by Two High School Students](https://www.bilibili.com/video/BV1391vBAEuz)

## 一些碎碎念 / Notes
有问题问我，别太去打扰美术妹子 / Ask me if you have questions, please don't bother the art girl too much  
只是一个普通外行的代码，质量很差，没有什么架构，可以学习不能照搬，issue可能不会看 /
Just an amateur's code with poor quality and no architecture, you can learn from it but don't copy directly, issues may not be checked

---

*基于上海爱丽丝幻乐团的东方Project系列，本作品为粉丝创作，非官方产品* /
*Based on Team Shanghai Alice's Touhou Project series. This is a fan creation, not an official product*